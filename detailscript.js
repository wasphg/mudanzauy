//import { data_products } from 'datascript.js';
const data_products = {
    products: [
      {
        Info: {
  ...
  function createDetailProduct(productId) {  
    const productFound = data_products.products.find(product => product.Info.id == productId);  
    if (!productFound) {    
      return;
    }
    //console.log(productFound.Info.title)
    ...
    productFound.Info.images.forEach(myImageUrl => {
      const mySpan = document.createElement("span")
      mySpan.classList.add("image")
      mySpan.classList.add("main")
   ...
   function displayDetailProduct(productId) {
    ...
    if (sectionContainer) { // Check if sectionContainer is not null
      productContainer.appendChild(sectionContainer);
    } else {
    // Handle the case where sectionContainer is not yet created
      console.error("Section container not created");
    }
  }
